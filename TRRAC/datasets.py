...
def get_datasets(dataframe_fp, subject_keys, test_val_split, predictors, targets, environments, exclude, remove_keys, fold_selection):
    """
    ...
    if len(targets) == 1:
        train, test = subjID_sensitive_datasplit_balanced(dataframe, test_size=test_perc, key=subject_keys, target=targets, fold_selection=fold_selection)
        # Split train into into train and val by subject
        train, validation = subjID_sensitive_datasplit_balanced(train, test_size=val_perc, key=subject_keys, target=targets, fold_selection=fold_selection)
    else:
...
def get_datasets_for_experiment(config):
    ...
    environment_datasets, val_dataset, test_dataset = get_datasets(
        ...
        fold_selection=data_config.get('fold_selection', None))
