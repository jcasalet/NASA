...
RNAS_DIR = 'RNAseqDB/data/normalized' # changed path LMS
...
def gamma_coef(x, y):
    ...
    print('x = ', x)
    print('y = ', y)
    print('dists_x in gamma_coef shape = ', dists_x.shape)
    print('dists_y in gamma_coef shape = ', dists_y.shape)
...
def gamma_coefficients(expr_x, expr_z):
    ...
    # JC
    print('dists_x shape = ', dists_x.shape)
    print('dists_z shape = ', dists_z.shape)
...
def compute_tf_tg_corrs(expr, gene_symbols, tf_tg=None, flat=True):
    ...
    for tf, tgs in tf_tg.items():
        ...
        if tf in gene_symbols and len(tg_idxs) > 0:
            ...
            expr_tf = expr[:, tf_idx]
            # JC
            print('expr_tgs shape = ', expr_tgs.shape)
            print('expr_tf shape = ', expr_tf)
...
def compare_cophenetic(l_matrix1, l_matrix2):
    ...
    # JC
    print('dists1 shape = ', dists1.shape)
    print('dists2 shape = ', dists2.shape)
...
#liver_data = drive.CreateFile({'id':'1L5NnQy-4xgBb33osCHsPi7w_b0z0EELI'})
#liver_data.GetContentFile('Normalized.CRISP.Liver.Symbol.Filtered.041521.csv')


#info_data = drive.CreateFile({'id':'16wGV18-c2XEHnwB9YAUWJx8mJ2NhzlNU'})
#info_data.GetContentFile('metadata_wbgan.csv')
info_data = pd.read_csv('./data/metadata_wbgan.csv',
                        sep=',', header=0, index_col=None)

#folder_metadata = {'title' : 'models_dir', 'mimeType' : 'application/vnd.google-apps.folder'}
#models_dir = drive.CreateFile(folder_metadata)
#models_dir.Upload()
models_dir = './models'

####################
# read in gl liver data
#####################
...
if __name__ == '__main__':
    ...
    # read in wgban csv
    ######################3
    info_df = pd.read_csv('./data/metadata_wbgan.csv', index_col=0)
    print(info_df.shape)
    info_df.head()
    ...
    CONFIG = {'gpu': 3,
              'epochs': 2000,
              'latent_dim': 64,  # Number of input units (gets passed to make_generator() as z_dim) LMS
              'batch_size': 32,
              'nb_layers': 2,
              'hdim': 256,  # Number of units for each hidden layer (gets passed to make_generator() ) LMS
              'lr': 5e-4,  # 5e-4,
              'nb_critic': 5}

    # Load dataset
    expr_df = pd.read_csv('./data/liver_with_flight.tsv', sep='\t', index_col=0).drop(columns=['env'])  # JC
    '''x = expr_df.values.T 
    symbols = expr_df.index.values
    sampl_ids = expr_df.columns.values'''
    x = expr_df.values
    symbols = expr_df.columns.values
    sampl_ids = expr_df.index.values
    print('sample_ids: ', sampl_ids)
    print('len of samples = ', str(len(sampl_ids)))
    ...
    print('tissues: ', tissues.shape)
    print('Cat covs: ', cat_covs.shape)

    # Process numerical metadata
    # JC
    '''num_cols = ['oro_percent_positivity'] # LMS
    num_covs = info_df.loc[sampl_ids, num_cols].values # LMS
    num_covs = standardize(num_covs)
    num_covs = np.float32(num_covs)'''
    num_covs = list()
    '''for i in range(cat_covs.shape[0]):
        if info_df.iloc[i]['oro_percent_positivity'] != 0:
            num_covs.append([info_df.iloc[i]['oro_percent_positivity']])
        else:
            num_covs.append([np.random.random_sample() * 20.])'''
    for i in range(len(info_df)):
        num_covs.append([info_df.iloc[i]['oro_percent_positivity']])
    num_covs = np.array(num_covs)
    print('Num covs: ', num_covs.shape)

    # num_covs = np.array([[i] for i in info_df['oro_percent_positivity'].values])
    # num_covs = np.ones((x.shape[0], 1), dtype=np.float32)
    #print('num_covs: ', num_covs)
    #print('cat_covs: ', cat_covs)
    ...
    print('got past make_generator')
    disc = make_discriminator(x_dim, vocab_sizes, nb_numeric,
                              h_dims=[CONFIG['hdim']] * CONFIG['nb_layers'])

    print('got past make discriminator')
