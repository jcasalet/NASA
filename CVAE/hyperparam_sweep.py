...
betas = [0.01, 0.05, 0.1, 0.25] # go higher?
latent_sizes = [16, 32, 64, 128] # maybe 128?
...
for lr, wd, beta, latent_size in itertools.product(learning_rates, weight_decay, betas, latent_sizes):
    ...
    print(f"Hyperparameters: lr={lr}, weight_decay = {wd}, beta={beta}, latent_size={latent_size} -> Avg Val Loss: {avg_val_loss}")

# Find the best hyperparameters based on validation loss
best_van_hyperparams = min(vanilla_results, key=lambda x: x[4])
print(f"Best hyperparameters: Learning Rate: {best_van_hyperparams[0]}, Weight decay: {best_van_hyperparams[1]}, Beta: {best_van_hyperparams[2]}, Latent Size: {best_van_hyperparams[3]}")

with open('best_hyperparameters.txt', 'w') as file:
    file.write(f"Best vanilla hyperparameters: Learning Rate: {best_van_hyperparams[0]}, Weight decay: {best_van_hyperparams[1]}, Beta: {best_van_hyperparams[2]}, Latent Size: {best_van_hyperparams[3]}")
