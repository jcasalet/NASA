...
for adata in adata_list:
    ...
    adata.obs['batch'] = adata.obs_names.map(utils.extract_batch)
    adata.obs['batch'] = adata.obs['batch'].astype(int)
    adata = utils.preprocess_adata(adata, n_top_genes=2000)
...
adata_combined = sc.read_h5ad('data/corrected_data.h5ad')

sc.tl.pca(adata_combined, svd_solver='arpack')

# Visualize the PCA results, coloring by an observation variable
sc.pl.pca(adata_combined, color='dataset', size = 10)
