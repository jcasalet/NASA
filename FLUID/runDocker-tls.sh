...
case $ROLE in
	agg)
		...
		docker run --net $MYNET -h ${HOSTNAME} --rm  --user=fluid:fluid  -v ${DATA_PATH}:/data:rw  -v ${SCRIPT_PATH}:/scripts:ro --ip ${IP_ADDR}  --add-host agg:192.168.56.101  --add-host colab1:192.168.56.102  --add-host colab2:192.168.56.103  ${IMAGE_NAME}  /scripts/runCrisp-tls.sh -r $ROLE
		;;
	colab1)
		HOSTNAME=colab1
