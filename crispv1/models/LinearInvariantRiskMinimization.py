...
class LinearInvariantRiskMinimization(object):
    ...
    def train(self):
        ...
        for iteration in range(self.args["n_iterations"]):
            penalty = 0
            error = 0
            error_dict[iteration] = list()
            penalty_dict[iteration] = list()
            for env_loader in self.train_loaders:
                weight_list = list()
                for inputs, targets in env_loader:
                    inputs = torch.cat((torch.ones(inputs.size(0), 1), inputs), 1)  ##
                    inputs_np = inputs.numpy()
                    '''for col in range(inputs_np.shape[1]):
                        val_list = list()
                        for row in range(inputs_np.shape[0]):
                            val_list.append(inputs[row][col].item())
                        if max(val_list) == 0:
                            weight_list.append(0)
                        else:
                            weight_list.append([1/max(val_list)])
                    self.w = torch.tensor(weight_list)
                    self.w.requires_grad = True'''
                    if self.cuda:
                        inputs = inputs.cuda()
                    error_e = loss(inputs @ self.phi @ self.w, targets)
                    #error_e = loss(softmax(inputs @ self.phi @ self.w), targets)
                    error_dict[iteration].append(error_e.item())
                    #error_e = loss((inputs @ self.phi @ self.w)/np.max(np.array(inputs)), targets)
                    penalty += torch.autograd.grad(error_e, self.w, create_graph=True)[0].pow(2).mean()
                    penalty_dict[iteration].append(penalty.item())
                    error += error_e
        ...
        '''for j in range(iteration):
            ...
            plt.figure()
            x = [i for i in range(len(penalty_dict[j]))]
